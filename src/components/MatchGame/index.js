...
class MatchGame extends Component {
  ...
  componentDidMount = () => {
    ...
    const {time} = this.state
    if (time !== 0) {
      this.intervalId = setInterval(this.decreaseSecond, 1000)
    }
  }
  ...
  updateImageListIndex = thumbId => {
    console.log('thumbnail clicked')

    const {time, score, imageListIndex} = this.state
    const randomIndex = Math.floor(Math.random() * imagesList.length)
    let updatedScore = score

    if (imagesList[imageListIndex].id === thumbId) {
      updatedScore += 1
      this.setState({
        imageListIndex: randomIndex,
        score: updatedScore,
      })
    } else {
      clearInterval(this.intervalId)
  ...
  renderGameBody = () => {
    console.log('render game body')

    const {activeTabId, imageListIndex, time} = this.state
    const filteredImagesList = imagesList.filter(
      eachImg => eachImg.category === activeTabId,
    )
    const matchImgUrl = imagesList[imageListIndex].imageUrl
    return (
      <div className="running-game-body-container">
        <div className="fruit-img-container">
          <img src={matchImgUrl} alt="match" className="fruit-img" />
        </div>
        ...
        <ul className="fruits-item-container">
          {filteredImagesList.map(eachFruit => (
            <FruitItem
              key={eachFruit.id}
              fruitDetails={eachFruit}
              updateImageListIndex={this.updateImageListIndex}
            />
          ))}
        </ul>
      </div>
  ...
  render() {
    const {isGameInProgress, score, time} = this.state
